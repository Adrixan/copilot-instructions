...
- **Error handling:** Always check errors. Wrap with `fmt.Errorf("context: %w", err)` for stack context. Use `errors.Is`/`errors.As` for matching.
- **Concurrency:** Prefer channels and `sync` primitives. Use `context.Context` for cancellation and timeouts on all I/O operations.
